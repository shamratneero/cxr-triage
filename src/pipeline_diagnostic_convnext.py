"""
Pipeline Diagnostic Script — ConvNeXt variant
=================================================
Same checks as the DenseNet version, targeting ConvNeXt before
committing to an overnight training run.

Usage (from D:\\cxr-triage, with cxr conda env active):
    python src/pipeline_diagnostic_convnext.py
...
def main():
    print("CXR TRIAGE — CONVNEXT PIPELINE DIAGNOSTIC")
    print("Run before starting the overnight ConvNeXt training run.\n")
    ...
    if r1 and r3 and r4 and r5:
        print("\nALL CHECKS PASSED. Safe to start the overnight ConvNeXt run.")
    else:
        print("\nONE OR MORE CHECKS FAILED. Fix before starting training —")
        print("do not run overnight with a failing check.")
