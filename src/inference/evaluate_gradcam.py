...
from src.inference.gradcam import GradCAM  # norm5-targeted, validated
...
IMAGE_SIZE      = 320
USE_CLAHE       = True
IOU_PERCENTILE  = 90   # locked methodology — matches full-run validated results
...
def main():
    ...
    # Track both recognized-positive and all-positive results separately
    results_recognized = {label: [] for label in LABEL_MAP}
    results_all = {label: [] for label in LABEL_MAP}
    errors = 0
    ...
    for _, row in tqdm(bbox_pa.iterrows(), total=len(bbox_pa),
        ...
        try:
            ...
            iou     = compute_iou(heatmap, bbox, percentile=IOU_PERCENTILE)

            results_all[finding].append(iou)
            if prob >= CONFIDENCE_THRESHOLD:
                results_recognized[finding].append(iou)
    ...
    def summarize(results, label_str):
        print(f"\n{label_str}")
        print(f"{'Finding':<20} {'Count':<8} {'Mean IoU':<12} "
        ...
        print("-" * 65)
        if len(all_ious) > 0:
            print(f"{'Overall':<20} {len(all_ious):<8} {all_ious.mean():<12.4f} "
    ...
    output_recognized = summarize(results_recognized, "=== RECOGNIZED POSITIVES (prob >= 0.5) ===")
    output_all = summarize(results_all, "=== ALL POSITIVES (regardless of confidence) ===")

    with open(OUTPUT_PATH, 'w') as f:
        json.dump({'recognized_positives': output_recognized, 'all_positives': output_all}, f, indent=2)
