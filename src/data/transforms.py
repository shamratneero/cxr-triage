...
def get_train_transforms(image_size=224, use_clahe=False):
    transform_list = [transforms.Resize((image_size, image_size))]
    if use_clahe:
        transform_list.append(CLAHETransform(clip_limit=2.0, tile_size=8))
    transform_list += [
...
def get_val_transforms(image_size=224, use_clahe=False):
    transform_list = [transforms.Resize((image_size, image_size))]
    if use_clahe:
        transform_list.append(CLAHETransform(clip_limit=2.0, tile_size=8))
    transform_list += [
